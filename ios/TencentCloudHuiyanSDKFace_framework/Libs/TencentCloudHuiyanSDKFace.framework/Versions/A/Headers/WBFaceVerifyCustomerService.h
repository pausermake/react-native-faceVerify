...
@return sdkInfo为和后台交互需要的字段，内容如下：
 @{
     @"encryptedAESKey" : @"",
     @"identityStr" : @""
 };
*/
-(void)initCusSDKWithAppId:(nullable NSString *)appId
                   orderNo:(NSString *)orderNo
                   licence:(NSString *)licence
                 sdkConfig:(WBFaceVerifySDKConfig *)sdkConfig
                   success:(void (^)(NSDictionary * _Nonnull sdkInfo))success
                   failure:(void (^)(WBFaceError * _Nonnull error))failure;

/*
@breif 定制SDK拉起页面
@param metaData 后台交互获取的envInfo 格式：@{@"envInfo" : envInfo}
@return error 拉起成功则返回空，其他情况返回具体错误
*/
- (WBFaceError *)startFaceVerifyWithMetaData:(NSDictionary *)metaData;
